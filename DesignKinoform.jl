...
for i in 1:total_cycles
	...
	#@show gˈmodulus_diff = round(sum_sq_diff(abs.(gˈ[i]), 1.0), sigdigits = 3)
	#@show gˈ_modulus_is_1 = all(abs.(gˈ[i]) .== 1.0)

	#@show sizes_match = (size(g[i]) == size(G[i]))

	G_raw = F(g[i - 1]) # uncorrected propagation.
	@show correction_diff = round(sum_sq_diff(abs.(G_raw), abs.(G[i])), sigdigits = 3)

	@show insanity_modulus_diff = round(sum_sq_diff(abs.(G_raw), abs.(G[0])), sigdigits = 3)
	@show insanity_phase_diff = round(sum_sq_diff(angle.(G_raw), angle.(G[0])), sigdigits = 3)
	
	G_errors[i] = sum_sq_diff(abs.(G[0]), abs.(G[i]))
